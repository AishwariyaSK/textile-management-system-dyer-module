...
const NotificationCard = ({id, userId, isForAdmin, batchId, message, seen, dateTime, category, onDelete}) => {
    ...
    const handleView=async () => {
        try{
            ...
            } else if (category === 'status update') {
    ...
    return (
        <div className={`border grid grid-cols-4 p-4 place-items-center rounded-md shadow-sm mb-4 ${seen ? 'bg-white' : 'bg-blue-50'}`}>
          <div className={`grid-3/5 text-gray-800 ${!seen ? 'font-bold' : 'font-normal'}`}>
            {message}
          </div>
